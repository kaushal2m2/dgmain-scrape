# scrapes comptia's security forums, getting the date, post title, post body, and top 5 responses for each
...
# url for comptia  
url = "https://www.comptia.com/topic/security/"
...
with open('questions_comptia.csv', 'a') as f:
    writer = csv.writer(f)
    ...
    while(recent):
        ...
        for j in range(1, 16):  # 15 posts per page
            ...
            if diff[j-1] <= maxdays:  # if its recent enough open the post
                ...
                title = sp.title.string.strip().strip(" -  Comptia")
