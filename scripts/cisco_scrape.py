# scrapes cisco's security forums, getting the date, post title, post body, and top 5 responses for each
...
# url for cisco security
url = "https://community.cisco.com/t5/security/ct-p/4561-security"
...
with open('questions_cisco.csv', 'a') as f:
    writer = csv.writer(f)
...
#         dat = soup.find_all('time', datetime='MM-dd-yyyy hh:mm a') THIS LINE GETS CHANGED
        ...
        for j in range(1, 16):  # 15 posts per page
            ...
            if diff[j-1] <= maxdays:  # if its recent enough open the post
                ...
                title = sp.title.string.strip().strip(" - Cisco Community")
                ...
                if len(bod) > 1:
                    for k in range(1, len(bod)):
                        if(k > 5):  # capped at 5 (best) responses per post
                            break
                        rs = ""
